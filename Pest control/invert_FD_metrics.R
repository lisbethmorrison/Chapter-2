...
## read in trait data 
effect_traits <- read.csv("../Data/Trait data/Pest control/invert_effect_traits.csv", header=TRUE)
response_traits <- read.csv("../Data/Trait data/Pest control/invert_response_traits.csv", header=TRUE)
effect_both_traits <- read.csv("../Data/Trait data/Pest control/invert_effect_both_traits.csv", header=TRUE)
response_both_traits <- read.csv("../Data/Trait data/Pest control/invert_response_both_traits.csv", header=TRUE)
all_traits <- read.csv("../Data/Trait data/Pest control/invert_all_traits.csv", header=TRUE)
BBS_data <- read.csv("../Data/Analysis_data/Pest control/BBS_2004_2018_invert.csv", header=TRUE)
...
all_effect_traits <- all_traits[,c(1,8:11)] ## keep name and effect traits
...
#############################################################################################
## First run this on species with effect traits (n=48)
## PCA of all effect traits
effect <- prcomp(effect_traits[ ,2:5]) ## bill length, width and depth, gape width and tarsus length
summary(effect) # PC1 accounts for 74.6% of the total variation and PC2 15.4% of the total variation
...
effect_eigen ## PCA1 eigenvalue = 2.98, PCA2 eigenvalue = 0.61
...
spp_list <- data.frame(ENGLISH_NAME = unique(effect_traits$ENGLISH_NAME)) ## 48 species
BBS_data_effect <- merge(spp_list, BBS_data, by="ENGLISH_NAME")
BBS_data_effect <- droplevels(BBS_data_effect)
length(unique(BBS_data_effect$ENGLISH_NAME)) ## 48 species
...
#############################################################################################
## Now run this on species with all traits (n=42)
## PCA of all effect traits
effect2 <- prcomp(all_effect_traits[ ,2:5]) ## bill length, width and depth, gape width and tarsus length
summary(effect2) # PC1 accounts for 75% of the total variation and PC2 14% of the total variation
## use these two axes to calculatse FD of effect traits
...
effect_eigen2 ## PCA1 eigenvalue = 2.99, PCA2 eigenvalue = 0.57
## Technically only need PCA1 to explain maximum amount of variation
...
## merge effect traits with BBS species data to match number of species
spp_list2 <- data.frame(ENGLISH_NAME = unique(all_effect_traits1$ENGLISH_NAME)) ## 42 species
...
######### 48 species which ONLY have effect traits
...
## save file
write.csv(functional_div_effect1, file="../Data/Analysis_data/Pest control/FD_effect_48spp.csv", row.names=FALSE)

######### 42 species which have ALL traits
# find distance matrix 
...
# dataframe with FD values and gridrefs (199 in total) for 42 species
...
## save file
write.csv(functional_div_effect2, file="../Data/Analysis_data/Pest control/FD_effect_42spp.csv", row.names=FALSE)
...
write.csv(FD_multi_effect, file="../Data/Analysis_data/Pest control/FD_multi_effect_48spp.csv", row.names=FALSE)

#################################################################################################################
## Now run this on species with effect and response traits (n=42)
...
## this is calculated on the two PCA axes derived from 5 effect traits
write.csv(FD_multi_effect_response, file="../Data/Analysis_data/Pest control/FD_multi_effect_42spp.csv", row.names=FALSE)
...
all_response_traits <- all_traits[,c(1,12:17)] ## keep name and response traits
...
response_corr2
## not very correlated, highest r = -0.63
...
summary(response) ## PC1 accounts for 30% of the total variation and PC2 20.7% of the total variation, PCA3 18.4%

response_eigen <- get_eigenvalue(response)
response_eigen ## PCA1 eigenvalue = 1.89, PCA2 eigenvalue = 1.24, PCA3 eigenvalue = 1.1 (keep all 3 as they are above 1)
...
x_corr2 
## PCA1 correlates with mean latitude and STI
## PCA2 correlates with SSI and brood size
## PCA3 correlates with clutch size
...
spp_list <- data.frame(ENGLISH_NAME = unique(response_traits$ENGLISH_NAME)) ## 63 species
BBS_data_response <- merge(spp_list, BBS_data, by="ENGLISH_NAME")
BBS_data_response <- droplevels(BBS_data_response)
length(unique(BBS_data_response$ENGLISH_NAME)) ## 63 species with trait and abundance data
...
summary(response2) ## PC1 accounts for 33% of the total variation and PC2 20.6% of the total variation, PCA3 18.5%

response_eigen2 <- get_eigenvalue(response2)
response_eigen2 ## PCA1 eigenvalue = 2.01, PCA2 eigenvalue = 1.23, PCA3 eigenvalue = 1.11
...
## add in numbers 1:199 (42 species reduces number of sites to 199)
site_match2$site <- 1:199
## remove first column (which is actual gridrefs) and leave sites as numbers 1-200
...
## save file
write.csv(functional_div_response, file="../Data/Analysis_data/Pest control/FD_response_63spp.csv", row.names=FALSE)

################################################
## First run this on species with both effect and response traits (n=42)
...
## save file
write.csv(functional_div_response2, file="../Data/Analysis_data/Pest control/FD_response_42spp.csv", row.names=FALSE)
...
## First run this on species with response traits (n=63)
...
## this is calculated on the two PCA axes derived from 5 effect traits
write.csv(FD_multi_response, file="../Data/Analysis_data/Pest control/FD_multi_response_63spp.csv", row.names=FALSE)
## all 200 sites and 63 species
...
## this is calculated on the two PCA axes derived from 5 effect traits
write.csv(FD_multi_response_effect, file="../Data/Analysis_data/Pest control/FD_multi_response_42spp.csv", row.names=FALSE)
## 199 sites and 42 species
...
## We want to combine both traits and effect traits => calculate FD metrics using these (n=48)
## Then combine both and response traits => calculate FD metrics using these (n=63)
## Repeat this with subset of species (n=42)

## select both only traits from all traits file
all_effect_both_traits <- all_traits[,c(1,3:6,8:11)] ## keep name and both traits (42spp)
effect_both_traits <- effect_both_traits[,-c(2)] ## remove CBC code (48spp)
...
################################################
## Sort out data on species with only effect AND both traits (n=48)
## PCA of all both traits
effect_both <- prcomp(effect_both_traits[ ,3:10]) ## all response traits
summary(effect_both) ## PC1 accounts for 69% of the total variation and PC2 13% of the total variation

effect_both_eigen <- get_eigenvalue(effect_both)
effect_both_eigen ## PCA1 eigenvalue = 5.53, PCA2 eigenvalue = 0.1.06 (take the first two axes)
...
################################################
## Sort out data for species with all traits (n=42)

## PCA of all response traits
effect_both2 <- prcomp(all_effect_both_traits[ ,2:9]) ## all response traits
summary(effect_both2) ## PC1 accounts for 68% of the total variation and PC2 12% of the total variation

effect_both_eigen2 <- get_eigenvalue(effect_both2)
effect_both_eigen2 ## PCA1 eigenvalue = 5.4, PCA2 eigenvalue = 0.94 (take first two)
...
################################################
## First run this on species with effect and both traits (n=48)
...
## save file
write.csv(functional_div_effect_both, file="../Data/Analysis_data/Pest control/FD_effect_both_48spp.csv", row.names=FALSE)
...
## save file
write.csv(functional_div_effect_both2, file="../Data/Analysis_data/Pest control/FD_effect_both_42spp.csv", row.names=FALSE)
...
## this is calculated on the two PCA axes derived from 5 effect traits
write.csv(FD_multi_effect_both, file="../Data/Analysis_data/Pest control/FD_multi_effect_both_48spp.csv", row.names=FALSE)
## all 200 sites and 48 species
...
## this is calculated on the two PCA axes derived from 5 effect traits
write.csv(FD_multi_all_effect_both, file="../Data/Analysis_data/Pest control/FD_multi_effect_both_42spp.csv", row.names=FALSE)
## 199 sites and 42 species
...
## select both only traits from all traits file
all_response_both_traits <- all_traits[,c(1,3:6,12:17)] ## keep name and both traits (28spp)
...
################################################
## Sort out data on species with only response AND both traits (n=63)
## PCA of all both traits
response_both <- prcomp(response_both_traits[ ,2:11]) ## all response traits
summary(response_both) ## PC1 accounts for 37% of the total variation and PC2 17.5% of the total variation, PCA3 13.8%

response_both_eigen <- get_eigenvalue(response_both)
response_both_eigen ## PCA1 eigenvalue = 3.72, PCA2 eigenvalue = 1.75, PCA3 = 1.38, PCA4 = 1.07 (take the first 4 axes)
...
## merge response traits with BBS species data to match number of species
spp_list <- data.frame(ENGLISH_NAME = unique(response_both_traits$ENGLISH_NAME)) ## 63 species
BBS_data_response_both <- merge(spp_list, BBS_data, by="ENGLISH_NAME")
BBS_data_response_both <- droplevels(BBS_data_response_both)
length(unique(BBS_data_response_both$ENGLISH_NAME)) ## 63 species with trait and abundance data
...
summary(response_both2) ## PC1 accounts for 38% of the total variation and PC2 18% of the total variation, PCA3 13%

response_both_eigen2 <- get_eigenvalue(response_both2)
response_both_eigen2 ## PCA1 eigenvalue = 3.8, PCA2 eigenvalue = 1.8, PCA3 eigenvalue = 1.35, PCA4 = 1.11 (take first 4)
...
## merge effect traits with BBS species data to match number of species
spp_list2 <- data.frame(ENGLISH_NAME = unique(all_response_both_traits$ENGLISH_NAME)) ## 42 species
...
length(unique(BBS_data_all_response_both$ENGLISH_NAME)) ## 42 species with trait and abundance data
...
################################################
## First run this on species with both and response traits (n=63)
...
# dataframe with FD values and gridrefs (200 in total) for 63 species
...
## save file
write.csv(functional_div_response_both, file="../Data/Analysis_data/Pest control/FD_response_both_63spp.csv", row.names=FALSE)
...
# dataframe with FD values and gridrefs (200 in total) for 42 species
...
## save file
write.csv(functional_div_response_both2, file="../Data/Analysis_data/Pest control/FD_response_both_42spp.csv", row.names=FALSE)
...
## First run this on species with response and both traits (n=63)
...
## this is calculated on the two PCA axes derived from 5 effect traits
write.csv(FD_multi_response_both, file="../Data/Analysis_data/Pest control/FD_multi_response_both_63spp.csv", row.names=FALSE)
## all 200 sites and 63 species
...
## this is calculated on the two PCA axes derived from 5 effect traits
write.csv(FD_multi_all_response_both, file="../Data/Analysis_data/Pest control/FD_multi_response_both_42spp.csv", row.names=FALSE)
## 199 sites and 42 species
